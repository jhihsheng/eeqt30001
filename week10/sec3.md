...
do — is to optimize the *structure* for the quantum Fisher information, and the
*quantum emitters that live inside it*.
...
The two lines have begun to merge: a Southampton series on the *flow* of Fisher
information in picometric optical metrology, one paper of which is joint with the
Rotter group (Weimar et al., arXiv:2508.13640, preprint).
<!-- ★ 開課前查核: Southampton FI-flow 系列（"Improving/Revealing topological
     features of/Controlling Fisher information flow in optical metrology"）
     卷期補全後升級為正式引用；Weimar et al. arXiv:2508.13640 為 preprint，
     僅作方向性引用（2026-09-21 裁定 F16a）。 -->
...
optimize the *light*; the classical structure end has just opened; the quantum
structure end and the emitter end (flag) are this lab's claim.
```
...
et al. 2022). To our knowledge (as of 2026), **no bound theory of Chao type exists for
$F_Q$ over structures**; the closest results are the disorder-average invariance
(Horodynski et al. 2021) and the FI continuity equation (Hüpfl et al. 2024). At the
same time, $F_\theta$ hands inverse design（反向設計）a ready-made objective function
— maximize the top eigenvalue of Eq. {eq}`eq-w10-ftheta` over *structures*, not
states. (One adjacent debate already proved the stakes: exceptional-point sensors show
diverging *response*, yet for the linear, reciprocal EP sensors analysed so far QFI
analysis found the noise diverges in step — classical response is not quantum
estimability, and QFI is the final judge (Wiersig 2014; Lau and Clerk 2018) — though
an EP accelerometer with enhanced signal-to-noise ratio (Kononchuk et al. 2022) shows
the last word is not in; extending the verdict to engineered nanophotonic environments
is a sister problem to the seeds below.) Three concrete openings, each sized for a
master's thesis:

1. **Inverse design for QFI — the quantum structure-end problem** (numerical + theory;
   pipeline: FMQA, factorization machine with quantum annealing). The tutorial solved
   "given the structure, optimize the state." The classical version of the dual —
   *given the estimation target, optimize the nanostructure* — has just been opened
   (Bouchet, Carminati, and Mosk 2020; Grant et al. 2026, preprint); its QFI version is
   open: use the top eigenvalue of $F_\theta$ (classical warm-up, then QFI) as the
   objective of topology optimization, and ask whether a Chao-type upper bound exists
   for it (Chao et al. 2022; Bouchet, Rotter, and Mosk 2021).
...
Each of these has only a sparse literature — that is the point. The groups that
...
far optimizes the light; the classical structure end has just been opened, while the
quantum structure end (QFI-targeted inverse design, unknown upper bounds) and the
emitter end (generator engineering) are open — the three thesis seeds live exactly
there.
```
...
3. J. Aasi et al. (LIGO Scientific Collaboration), *Enhanced sensitivity of the LIGO gravitational wave detector by using squeezed states of light*, Nat. Photon. **7**, 613 (2013).
...
5. D. Ganapathy et al. (LIGO O4 Detector Collaboration), *Broadband quantum enhancement of the LIGO detectors with frequency-dependent squeezing*, Phys. Rev. X **13**, 041021 (2023).
6. H. Yu et al., *Quantum correlations between light and the kilogram-mass mirrors of LIGO*, Nature **583**, 43 (2020).
7. S. L. Braunstein and C. M. Caves, *Statistical distance and the geometry of quantum states*, Phys. Rev. Lett. **72**, 3439 (1994).
8. C. L. Degen, F. Reinhard, and P. Cappellaro, *Quantum sensing*, Rev. Mod. Phys. **89**, 035002 (2017).
9. E. M. Purcell, *Spontaneous emission probabilities at radio frequencies*, Phys. Rev. **69**, 681 (1946).
10. N. I. Zheludev and G. Yuan, *Optical superoscillation technologies beyond the diffraction limit*, Nat. Rev. Phys. **4**, 16 (2022).
11. G. H. Yuan and N. I. Zheludev, *Detecting nanometric displacements with optical ruler metrology*, Science **364**, 771 (2019).
12. T. Liu et al., *Picophotonic localization metrology beyond thermal fluctuations*, Nat. Mater. **22**, 844 (2023).
13. D. Bouchet, S. Rotter, and A. P. Mosk, *Maximum information states for coherent scattering measurements*, Nat. Phys. **17**, 564 (2021).
14. M. Horodynski, D. Bouchet, M. Kühmayer, and S. Rotter, *Invariance property of the Fisher information in scattering media*, Phys. Rev. Lett. **127**, 233201 (2021).
15. J. Hüpfl et al., *Continuity equation for the flow of Fisher information in wave scattering*, Nat. Phys. **20**, 1294 (2024).
16. L. M. Rachbauer, D. Bouchet, U. Leonhardt, and S. Rotter, *How to find optimal quantum states for optical micromanipulation and metrology in complex scattering problems: tutorial*, J. Opt. Soc. Am. B **41**, 2122 (2024).
17. O. Lib and Y. Bromberg, *Quantum light in complex media and its applications*, Nat. Phys. **18**, 986 (2022).
18. P. Cameron et al., *Adaptive optical imaging with entangled photons*, Science **383**, 1142 (2024).
19. N. Treps et al., *A quantum laser pointer*, Science **301**, 940 (2003).
20. M. Tsang, R. Nair, and X.-M. Lu, *Quantum theory of superresolution for two incoherent optical point sources*, Phys. Rev. X **6**, 031033 (2016).
21. P. Lodahl et al., *Chiral quantum optics*, Nature **541**, 473 (2017).
22. P. Chao, B. Strekha, R. Kuate Defo, S. Molesky, and A. W. Rodriguez, *Physical limits in electromagnetism*, Nat. Rev. Phys. **4**, 543 (2022).
23. D. Bouchet, R. Carminati, and A. P. Mosk, *Influence of the local scattering environment on the localization precision of single particles*, Phys. Rev. Lett. **124**, 133903 (2020).
24. Grant et al., arXiv:2605.14595 (2026), preprint.
25. J. Wiersig, *Enhancing the sensitivity of frequency and energy splitting detection by using exceptional points*, Phys. Rev. Lett. **112**, 203901 (2014); experiments: W. Chen et al., Nature **548**, 192 (2017); H. Hodaei et al., Nature **548**, 187 (2017).
26. H.-K. Lau and A. A. Clerk, *Fundamental limits and non-reciprocal approaches in non-Hermitian quantum sensing*, Nat. Commun. **9**, 4320 (2018); W. Langbein, Phys. Rev. A **98**, 023805 (2018); M. Zhang et al., Phys. Rev. Lett. **123**, 180501 (2019).
27. R. Kononchuk, J. Cai, F. Ellis, R. Thevamaran, and T. Kottos, *Exceptional-point-based accelerometers with enhanced signal-to-noise ratio*, Nature **607**, 697 (2022).

<!-- ★ 開課前補全 Grant2026（ref. 24）作者縮寫與題名（2026-09-21 裁定 F16b 僅給 arXiv 號）。 -->
