...
all $N!$ photon paths cannot be factorized, so for ideal (lossless) boson sampling with
$N\ge50$ photons no classical computer is believed able to sample from this
distribution (QO_note_6, Sec. 1).
...
<!-- GKP one-liner only (teacher decision 2026-07-20); reference GKP 2001.
     Industry data #3 (2026-09-21 ruling F8): on-chip GKP states: Larsen et al.,
     Nature 642, 587 (2025); first optical GKP states (free space):
     Konno et al., Science 383, 289 (2024). Secondary:
     ...
     Industry data #5 (2026-09-21 ruling F2): SPAC merger with Crane Harbor
     Acquisition Corp.; listed 2026-03 on Nasdaq + TSX (XNDU), ~US$302 million
     raised; first pure-play photonic QC company to trade publicly. Primary:
     Xanadu investor-relations press release (xanadu.ai, 2026-03) and SEC
     filings (EDGAR: Crane Harbor Acquisition Corp.). Secondary:
...
demonstrated on-chip generation of GKP states (Larsen et al. 2025; the first optical
GKP states had been made in free space a year earlier, Konno et al. 2024) — in one
sentence, GKP (Gottesman, Kitaev, and Preskill 2001) is the encoding that folds a
continuous-variable mode into a discrete qubit, and one sentence is all the depth we
need. Then came the Aurora modular system, and, in March 2026, a SPAC merger (with
Crane Harbor Acquisition Corp.) that listed Xanadu on Nasdaq and the TSX under the
ticker XNDU, raising about US\$302 million — the first pure-play photonic
quantum-computing company to trade publicly.

<!-- Industry data #6 (2026-09-21 ruling F5): Quandela/ORCA QD-source route
     (secondary: https://thequantuminsider.com/2026/03/24/11-companies-lighting-up-the-quantum-photonics-sector/ );
     QBoson = CIM-type special-purpose optical machines, Shenzhen plant. Primary:
     Shenzhen Municipal Government English site, https://www.sz.gov.cn/en_szgov/
     (the businesswire URL formerly listed here was misattributed and is removed). -->
```{table} Three photonic bets, side by side
:name: tab-w9-industry
| | **PsiQuantum** | **Xanadu** | **Quandela / ORCA** | **China** |
|---|---|---|---|---|
| encoding | DV (dual rail) | CV (GBS $\to$ GKP) | DV (QD single photons) | GBS (Jiuzhang); CIM-type (QBoson) |
| architecture | fusion-based lattice | modular GBS network | processors around QD sources | USTC/Jiuzhang line (GBS); QBoson special-purpose optical machines |
| signature bet | fab-first, no NISQ machine (GlobalFoundries) | error-corrected CV (Aurora, XNDU) | source quality | advantage demos (USTC); Shenzhen plant (QBoson) |
```
...
Eq. {eq}`eq-w9-haf`); Jiuzhang and Borealis turned that hardness into
quantum-advantage demonstrations, and Aurora turned the platform into a modular,
error-correction-oriented roadmap (GKP). Advantage, however, is a complexity claim —
